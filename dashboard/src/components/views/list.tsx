                                        ...
                                        <li key={s.id} className="group">
                                            <article className="grid grid-cols-1 items-center gap-2 px-3 py-3 sm:grid-cols-[minmax(16rem,2fr)_minmax(12rem,3fr)_96px_140px_64px] sm:gap-3 sm:px-4">
                                                ...
                                                <div className="flex justify-end sm:justify-end">
                                                    ...
                                                    >
                                                        <IoMdExpand className={cn("h-4 w-4 text-muted-foreground transition-transform", expanded[s.id] && "rotate-180")} />
                                                    </button>
                                                </div>
                                            </article>
                                            {expanded[s.id] && (
                                                <div className="px-3 sm:px-4 pb-4 -mt-2">
                                                    <div className="rounded-md border bg-background/60 p-3">
                                                        ...
                                                        <div className="grid grid-cols-1 gap-2 text-[11px] text-muted-foreground sm:grid-cols-3">
                                                            <div><span className="opacity-70">Session ID:</span> <span className="break-all text-foreground/90">{s.id}</span></div>
                                                            <div><span className="opacity-70">Created:</span> <span>{formatTime(s.createdAt)} • {formatRelative(s.createdAt)}</span></div>
                                                            <div><span className="opacity-70">Updated:</span> <span>{formatTime(s.updatedAt)} • {formatRelative(s.updatedAt)}</span></div>
                                                        </div>
                                                        ...
                                                        <div className="mt-3 overflow-x-auto">
                                                            <table className="min-w-full text-left">
                                                                ...
                                                                <tbody className="text-xs">
                                                                    {(Array.isArray(s.tabs) ? (s.tabs as any[]) : []).map((t, idx) => {
                                                                        ...
                                                                        return (
                                                                            <tr key={idx} className="border-b last:border-b-0 align-top">
                                                                                ...
                                                                                <td className="py-2 pr-3 whitespace-nowrap text-muted-foreground">
                                                                                    {closedMs ? `${formatTime(closedMs)} • ${formatRelative(closedMs)}` : <span className="opacity-60">—</span>}
                                                                                </td>
